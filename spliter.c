...
/**
 * spliter_commands - splits command lines according to
...
int spliter_commands(data_myshell *datash, char *input)
{
	...
	seperators *head_s, *list_s;
	fline_list *head_l, *list_l;
	int loop;
...
char **split_line(char *input)
{
	...
	size_b = TOKEN_BUFSIZE;
	tokens = malloc(sizeof(char *) * (size_b));
	if (tokens == NULL)
	{
	...
	for (i = 1; token != NULL; i++)
	{
		...
		{
			size_b += TOKEN_BUFSIZE;
			tokens = _reallocdp(tokens, i, sizeof(char *) * size_b);
			if (tokens == NULL)
			{
